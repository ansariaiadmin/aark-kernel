...
if command -v docker &> /dev/null && docker compose ps 2>/dev/null | grep -q "Up"; then
  ...
  docker compose exec -T db pg_dump -U aark-kernel aark-kernel 2>/dev/null > /tmp/aark-kernel-db-$TIMESTAMP.sql || echo "DB dump fail — شاید DB خاموشه"
  ok "DB dump — /tmp/aark-kernel-db-$TIMESTAMP.sql"
else
...
if [ -f /tmp/aark-kernel-db-$TIMESTAMP.sql ]; then
  tar -rf $BACKUP_FILE /tmp/aark-kernel-db-$TIMESTAMP.sql 2>/dev/null || true
