...
if [[ "FastAPI + React" == *"Node"* ]] || [[ "FastAPI + React" == *"Next"* ]] || [[ "FastAPI + React" == *"Nest"* ]]; then
  ...
  if [ -f docker-compose.yml ]; then
    ...
    for i in {1..30}; do
      ...
      if command -v curl &> /dev/null; then
        if curl -sf http://localhost:8000/api/v1/health/ready >/dev/null 2>&1 || curl -sf http://localhost:3000 >/dev/null 2>&1 || curl -sf http://localhost:8000 >/dev/null 2>&1 || curl -sf http://localhost:8080 >/dev/null 2>&1; then
          echo ""
...
echo -e "  آدرس / URL: http://localhost:3000 (Frontend) و http://localhost:8000/docs (API)"
echo -e "  ورود / Login: JWT login via /api/v1/auth/login"
echo -e "  سلامت / Health: http://localhost:8000/api/v1/health/ready"
echo ""
