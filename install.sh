...
echo -e "${CYAN}"
cat <<'BANNER'
AARK Kernel — v3.1.0 — تاریکی روشن شد — پشتیبانی صفر
ترید + AI + ریسک + SMS + ناتیف — تاریکی روشن شد
AI + Exchange (Nobitex/paper trading) + SMS + Email + Telegram
BANNER
echo -e "${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  🧙‍♂️ جادوگر نصب AARK Kernel v3.1.0 — تاریکی روشن شد${NC}"
echo -e "${BLUE}  AI + Exchange (Nobitex/paper trading) + SMS + Email + Telegram${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${YELLOW}سلام! 👋 ترید + AI + ریسک + SMS + ناتیف — تاریکی روشن شد${NC}"
...
if [ "aark-kernel" = "aark-kernel" ]; then
...
if [ "aark-kernel" = "legal-platform" ]; then
...
if [ "aark-kernel" = "universal-document-os" ]; then
...
if [ "$TELEGRAM_ENABLED" = "yes" ]; then
  ...
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && command -v curl &> /dev/null; then
    ...
    if [ -n "$TELEGRAM_CHAT_ID" ]; then
      curl -sf -X POST -H "Content-Type: application/json" -d "{\"chat_id\":\"$TELEGRAM_CHAT_ID\",\"text\":\"🧪 تست AARK Kernel v3.1.0 — تاریکی روشن شد — $(date)\"}" https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage -o /dev/null 2>&1 && ok "Telegram پیام تست فرستاده شد — چک کن — تاریکی روشن شد" || err "Telegram پیام fail — Chat ID چک کن — ربات استارت کردی؟ — تاریکی روشن شد"
    fi
  fi
...
if [ "$SKIP_ENV" = "true" ]; then
  ...
  cat > .env <<EOF
# AARK Kernel — .env — v3.1.0 — تاریکی روشن شد — $(date)
...
# Exchange — با هزینه — تاریکی روشن شد
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  🎉 جادو تمام! AARK Kernel آماده — تاریکی روشن شد! 🎉${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "${BOLD}${BLUE}📍 دسترسی:${NC}"
echo -e "${GREEN}  🌐 http://localhost:3000 — ترید + AI + ریسک + SMS + ناتیف — تاریکی روشن شد${NC}"
