...
curl -X POST http://localhost:3000/api/auth/login \
  ...
  -d '{"email":"admin@aark-kernel.dev","password":"Admin@123"}'
```
...
{
  "token": "jwt-token-...",
  "user": { "email": "admin@aark-kernel.dev", "role": "admin" }
}
```
...
```json
{
  ...
  "providers": {
    ...
    "telegram": { "status": "ok", "bot": "@aark-kernel_notif_bot" }
  },
  "notifications": {
...
1. تلگرام → @BotFather → /newbot → اسم → یوزرنیم (مثل aark-kernel_notif_bot)
