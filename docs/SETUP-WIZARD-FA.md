# 🧙‍♂️ جادوگر نصب AARK Kernel — ترید + AI + ریسک — فوق ساده — پشتیبانی صفر — Zero Support
...
## 🎯 AARK Kernel — ترید + AI + ریسک چیه؟ به زبان ساده:

ربات ترید هوشمند که ضرر کنترل می‌کنه + paper trading تمرینی بدون پول واقعی + نمودار LineChart + OrderBook زنده مثل تابلو بورس
...
### 🤖 AI Provider — برای تحلیل بازار

**چیه؟** هوش مصنوعی برای تحلیل ریسک و بازار — مثل مشاور هوشمند

**گزینه‌ها:**
- **ollama** — لوکال رایگان — https://ollama.com/ → نصب → رایگان — بهترین برای شروع
- **openai** — GPT-4 — https://platform.openai.com/api-keys → sk-proj-...
- **anthropic** — Claude — https://console.anthropic.com/ → sk-ant-...
- **mock** — بدون AI واقعی — برای تست

**کجا بگیرم؟** Ollama رایگان — یا OpenAI API Key

### 💱 Exchange — صرافی — برای ترید واقعی

**چیه؟** جایی که ترید می‌کنی — مثل نوبیتکس — برای ترید واقعی نیاز به API Key

**گزینه‌ها:**
- **nobitex** — نوبیتکس ایرانی — https://nobitex.ir/panel/api → API Key + Secret
- **mock** — paper trading بدون پول واقعی — امن — پیش‌فرض — برای تست عالی

**اگر mock باشه:** paper trading — بدون پول واقعی — امن

### 📱 SMS Panel — پنل پیامکی — برای ناتیف ترید

**چیه؟** وقتی ترید انجام می‌شه یا ریسک بالا می‌ره پیامک می‌ده: "ترید BTC انجام شد"

**گزینه‌ها:**
- **ghasedak** — قاصدک — https://ghasedak.me/ → رایگان 50 تا
- **kavenegar** — کاوه‌نگار — https://kavenegar.com/
- **mock** — تو لاگ — برای تست



## 🔔 سیستم ناتیفیکیشن — Notification System — سقف 10/10 — جدید v3.0.0

**چیه؟** وقتی ترید می‌شه، ریسک بالا می‌ره، یا خطا میاد — خبر می‌ده

**کانال‌ها:**
1. **In-App** — همیشه روشن — تو داشبورد می‌بینی
2. **Email** — ایمیل می‌ره — ترید جدید
3. **SMS** — پیامک می‌ره — ریسک بالا
4. **Telegram** — ربات تلگرام — وقتی ترید یا خطا میاد تلگرام خبر می‌ده — رایگان

**ربات تلگرام چطور بسازم؟ — 1 دقیقه:**
1. تلگرام → @BotFather → /newbot → اسم → یوزرنیم (مثل aark_notif_bot)
2. توکن می‌ده — مثل 123456:ABC...
3. ربات رو استارت کن → پیام بده
4. https://api.telegram.org/bot<TOKEN>/getUpdates → chat_id

**کد:** `backend/app/services/notification/service.py` — سقف 10/10
...
**پشتیبانی صفر:** همه راهنماها همون‌جا بود — اگر بازم گیر کردی: https://github.com/ansariaiadmin/aark-kernel/issues
